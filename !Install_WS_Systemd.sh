#!/bin/bash
path="`pwd`/WSServer.py"
service="/lib/systemd/system/ApneaWS.service"
...
echo "[Unit]" >> $service
echo "Description=呼吸暂停检测，主程序" >> $service
echo "" >> $service
...
then
  ...
  systemctl enable ApneaWS
  echo "已创建服务，请运行 systemctl start ApneaWS 执行该服务"
  exit
else
